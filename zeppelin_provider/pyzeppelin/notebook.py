...
class Note:
    ...
    def get_content(self):
        content = []
        for p in self.paragraphs:
            content.append("Paragraph {0} is {1}.\nText:\n{2}\nResults:\n{3}\nAssociated job urls:\n{4}" \
        ...
        if 'runtimeInfos' in paragraph_json:
            ...
            if 'jobUrl' in runtime_infos_json:
                ...
                if 'values' in job_url_json:
                    ...
                    self.job_urls = list(map(lambda x: x['jobUrl'], filter(lambda x: 'jobUrl' in x, job_url_values)))
    ...
    def get_errors(self):
        if self.status != 'FINISHED':
            return "Paragraph {0} is failed.\n\nText: {1}\n\nResults:{2}\n\nAssociated job urls: {3}\n\nJson:{4}"\
                .format(self.id, self.text, '\n'.join(list(map(lambda x: x[1], self.results))), str(self._job_urls), self._paragraph_json)
